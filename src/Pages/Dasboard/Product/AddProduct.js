...
export default function AddProduct() {
  ...
  async function handleAddForm() {
    try {
      const res = await customAxios.post(`${Prod}/add`, dummyForm);
      setId(res.data.id);
    } catch (err) {
      console.log(err);
    }
  ...
  async function handleImagesChange(e) {
    ...
    for (let i = 0; i < imagesFiles.length; i++) {
      count.current++;
      data.append("image", imagesFiles[i]);  // i upload each one here in forloop, but if without upload it will be array
      ...
      <div className="card" style={{height: '100%'}}>
        ...
        >
          ...
          <div style={{ width: "87px"}}>
            <span onClick={() => handleDelete(index, img)} style={{position: 'absolute', right: '3px', top: '-2px', color: 'white', cursor: 'pointer'}}><FontAwesomeIcon icon={faXmark} /></span>
            <div className="progress" ref={(e) => (progressStyle.current[index] = e)} 
              // e = the first progress, number on and so on
            >
    ...
    <div className="custom-box-shadow w-100">
      ...
      >
        ...
        <div
          ...
          style={{
            border: "2px dashed " + (closeInput ? "gray" : "#0086fe"),
            cursor: !closeInput && "pointer",
          }}
        >
          <img
            src={"/upload.png"}
            alt="upload here"
            width={"100px"}
            style={{ filter: closeInput && "grayscale(1)" }}
          />
          <p
            style={{ color: closeInput ? "gray" : "#0086fe", marginBottom: 0 }}
          >
            Upload images
          </p>
        </div>
        <div className="row">{imagesShow}</div>
        <div className="mt-3 d-flex justify-content-center">
          <button disabled={closeInput} className="btn btn-primary py-2 px-4">
            Save
          </button>
        </div>
      </Form>
